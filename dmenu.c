...
// Size multiplier
static const float multiplier = 1.8;
...
drawmenu(void)
{
	...
	drw_setscheme(drw, scheme[SchemeNorm]);
	drw_text(drw, x, offset_top * 2, w, bh, lrpad / 2, text, 0); // text Input

	curpos = TEXTW(text) - TEXTW(&text[cursor]);
	if ((curpos += lrpad / 2 - 1) < w)
	{
		drw_setscheme(drw, scheme[SchemeNorm]);
		drw_rect(drw, x + curpos, offset_top * 2, 2, bh - 4, 1, 0); // text cursor
	}
	...
	else if (matches)
	{
		...
		{
			drw_setscheme(drw, scheme[SchemeNorm]);
			drw_text(drw, x, offset_top * 2, w, bh, lrpad / 2, "<", 0);
		}
		x += w;
		for (item = curr; item != next; item = item->right)
		...
		if (next)
		{
			w = TEXTW(">");
			drw_setscheme(drw, scheme[SchemeNorm]);
			drw_text(drw, mw - w, offset_top * 2, w, bh, lrpad / 2, ">", 0);
		}
	}
